...
const channel = sWlc[message.guild.id].channel
  if (message.content.startsWith(prefix + "setwlcc")) {
...
client.on("guildMemberAdd", member => {
        if(member.guild.id === "588396521213526034") {  // ايدي السيرفر
  const channel = member.guild.channels.find('id', '588403343710224390'); //ايدي الروم
if (!channel) return;
channel.send(`**<@${member.user.id}> Welcome To 2.B Server  ** ❤️ `)  
}});
client.on("guildMemberRemove", member => {
        if(member.guild.id === "588396521213526034") { 
  const channel = member.guild.channels.find('id', '588403343710224390'); 
if (!channel) return;
  channel.send(`**${member.user.tag} Left The Server  ** 😭 `) 
}});
client.on('guildMemberAdd', member=> {
    member.addRole(member.guild.roles.find("name","⌠ User ⌡")); //اسم ا��رتبة
    });
